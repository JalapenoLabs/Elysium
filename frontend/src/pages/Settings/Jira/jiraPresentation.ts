...
// The site as the API spells it. A pasted address usually carries a trailing slash, and
...
export function summarizeScope(isAll: boolean, names: string[]) {
  return {
    isAll,
    count: names.length,
...
// An allowlist of everything can never hold one, since it is not a list of ids.
export function findUnreachableNames(
  credential: JiraCredential,
  reachableProjects: JiraProject[],
  reachableBoards: JiraBoard[],
) {
  const projectIds = new Set(reachableProjects.map((project) => project.projectId))
  const boardIds = new Set(reachableBoards.map((board) => board.boardId))
  const names: string[] = []

  if (!credential.allProjects) {
    for (const project of credential.projects) {
      if (!projectIds.has(project.projectId)) {
        names.push(project.projectKey)
      }
    }
  }

  if (!credential.allBoards) {
